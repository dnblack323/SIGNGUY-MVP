...
- EC9 Phase 9I-I implements the previously deferred Digital Print minimum split using the recorded repository decision: `item_minimum` `$20` and `order_minimum` `$40`. The line calculator enforces `item_minimum * quantity`; Quote/Order orchestration applies the `$40` order minimum once per document against eligible Digital Print line subtotals. Independent focused review passed. No exemption model or Control Center UI was introduced. All of Phase 9I remains open until the separate Calculator Extraction, Money Normalization, and Standalone Licensing Readiness Audit is completed and required corrections are resolved.
