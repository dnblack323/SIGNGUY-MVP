...
| **EC7** | Inventory, Purchasing, Finance, and Reporting | **IN PROGRESS (7a+7b+7c+7d backend + partial 7d frontend delivered; regression pending; Vendor+Material detail + Physical Count + Transfer flows remain)** | Phase 7a-c per prior entries. Phase 7d: 13 curated reports + Custom Report Builder foundation + CSV export (formula-injection neutralized). Frontend: Inventory / Supply Center / Purchase Orders (list + detail with Receive dialog covering partial + complete receiving) / Expenses / Finance Dashboard (with per-metric "Explain this basis" popovers) / Tax Reports / Reports (curated preview+CSV+Custom builder). **Remaining before EC7 COMPLETE:** Vendor detail page, Material detail (with Material Cost History drawer), Physical Count wizard, Inventory Transfer flow, receiving-created movement visibility inline on PO detail, frontend automated tests, `testing_agent_v3_fork` full-stack regression. 215/215 backend tests. |
...
**Register last updated:** 2026-07 — EC7 phase 7d delivered (backend + frontend). Backend adds: curated reports registry with 13 reports across inventory/purchasing/expenses/finance/tax (each declaring `data_source`, `date_basis`, `calc_basis`, `columns`, `limitations`, and required `Perm`); Custom Report Builder foundation with 3 approved datasets and strict field/filter whitelisting (no raw queries, no cross-tenant reads, no hidden fields); CSV export service that neutralizes formula-injection prefixes with a leading single quote and formats `_cents` to two-decimal dollars; every export writes an audit event; preview capped at 500 rows, export capped at 25 000 rows. Frontend adds: `lib/ec7.js` helpers plus 7 pages — `InventoryPage` (Items / Materials / Movements / Locations tabs), `SupplyCenterPage` (catalog + synthetic-data banner + dev seed), `PurchaseOrdersPage` (list + submit dialog with per-click Idempotency-Key + cancel-with-reason dialog), `ExpensesPage` (state tabs + New expense dialog + Void with reason + Archive/Restore), `FinanceDashboardPage` (8 basis-badged metric cards + 3 trend charts + Top customers + Payment-method breakdown + limitations footer), `TaxReportsPage` (3 summary cards + 4 tabs including manual overrides and exemption records), `ReportsPage` (curated preview+CSV export + Custom Builder with field whitelist toggles + CSV export). All routes and Business & Finance / Shop Operations flyouts wired to the canonical EC1 shell. Backend 215/215 green (5 new phase-7d scenarios + 210 baseline; 0 regressions). Frontend smoke-tested via Playwright screenshot. **`testing_agent_v3_fork` full-stack regression PENDING — EC7 remains IN PROGRESS until it passes.** Master plan Appendix A.4 (REVISED 2026-07) commercial authority remains locked and assigned to EC11/EC12/EC13; **no commercial code lands in EC7**. EC3.1 + EC6.3 still visible + pending. EC8 NOT started. EC14 gates on EC11 / EC12 / EC13 completion.
