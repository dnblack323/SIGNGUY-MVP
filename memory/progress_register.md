...
| **EC8** | Team, Scheduling, Time, Payroll, Employee Portal, Equipment Training & Certification | **IN PROGRESS — Phase 8a + 8b + 8c + 8d + 8e delivered** | Phase 8a-8c per prior entries. Phase 8d (Payroll): `PayPeriod`/`PayrollSnapshot`/`PayrollTransaction` append-only ledger, manager Payroll page, Employee Portal My Pay — 13/13 targeted tests. Phase 8e (Equipment/Training/Certification): `Equipment`/`TrainingDefinition`/`TrainingAssignment`/`QuizAttempt`/`PracticalSignoff`/`Certification` models; manager pages (Equipment list/detail, Training Definitions+Assignments incl. quiz authoring, Certification Matrix with issue/renew/revoke); Employee Portal My Training (quiz-taking UI) + My Certifications; Work Order assignment enforcement (hard block / warn+override / advisory role) surfaced verbatim in the `AssignDialog` UI, never re-implemented client-side — 15/15 targeted tests, verified further via live curl (block + warning-override paths) and screenshot smoke (Certification Matrix, My Training, quiz form, Work Order block/warning rendering). A real quiz-retry-after-fail bug and a stale-permission Employee Portal identity bug were found and fixed this phase. Evidence: `/app/evidence/EC8_evidence.md`. Phase 8f (final closure/full regression) NOT started — awaiting owner authorization. |
...
**EC8 Phase 8e complete and tested. Awaiting owner's explicit authorization for Phase 8f (final closure/full regression).**

- Do NOT begin Phase 8f implementation/full regression until the owner explicitly authorizes it.
- Do NOT begin EC9.
- Do NOT invoke or rerun the EC0–EC7 implementation prompts, or run `testing_agent_v4_fork`/a full regression before Phase 8f.

---

**Register last updated:** 2026-07 — **EC8 PHASE 8e COMPLETE (EC8 overall: IN PROGRESS).** Delivered Equipment/Training/Certification models + services + manager pages (Equipment, Training incl. quiz authoring, Certification Matrix with issue/renew/revoke) + Employee Portal self-service (My Training incl. quiz-taking, My Certifications) + Work Order assignment enforcement (hard block / warn+override / advisory-role), with the frontend rendering the backend's exact enforcement decision rather than re-implementing any rule. 15/15 targeted backend pytest green; two real bugs found and fixed (quiz retry-after-fail terminal-status guard; stale Employee Portal permissions on re-invite). Verified via targeted pytest + live curl (block/warning-override E2E) + screenshot smoke (Certification Matrix, My Training/quiz, Work Order requirements+assign dialog) — `testing_agent_v4_fork` intentionally NOT run this phase per the owner's credit-conservation instruction. Awaiting owner go-ahead for Phase 8f (final EC8 closure — full regression + full frontend suite).
