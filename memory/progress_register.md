...
| **EC6** | Asset Library, Proofs, Signatures, and Customer Portal | **CORRECTIONS REQUIRED** | Backend + 154/154 pytest **ACCEPTED** by owner. Two required deliverables still open: (a) Customer Portal Invoice Payment page (`/portal/invoices/:id/pay`) wired to EC4 Stripe Core (no parallel payment system, publishable-key + client_secret never leaked, webhook-confirmed reconciliation, idempotent duplicate-init protection); (b) automated frontend + end-to-end verification via `testing_agent_v3_fork` covering portal auth separation, portal identity/permissions, Portal Quotes/Orders/Invoices/Payments/Proofs/Signatures/Documents/Messages, public proof/sign/quote-request/customer-intake token flows, cross-tenant + cross-customer sweeps, and full EC1–EC5 regression. Also: verify signed-PDF boundary in docs (defer or implement), complete staff Asset Library / Proof / Approval / Signature UI surfaces (backend without functional staff UI does not satisfy EC6). Evidence to be regenerated after corrections. |
