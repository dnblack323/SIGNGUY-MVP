...
| **EC4** | Invoices, Payments, and Stripe Core | **CORRECTIONS REQUIRED** | Backend 134/134 tests pass. Security fix applied: Stripe publishable key + client_secret no longer rendered/logged/persisted. Frontend Stripe dialog uses a proper payment-form boundary + dev-only Simulate confirm button gated by AUTH_DEV_BYPASS. Automated frontend regression via `testing_agent_v3_fork` in progress. Evidence: `/app/evidence/EC4_evidence.md`. |
