...
| **EC7** | Inventory, Purchasing, Finance, and Reporting | **IN PROGRESS (phase 7a + 7b + 7c delivered)** | Phase 7a: Materials + Inventory + Movements. Phase 7b: Vendors + 80-SKU deterministic supplier catalog + Purchase Orders + Receiving. Phase 7c: Expenses + Categories (16 seeded) + labeled-basis Finance Dashboard (17 metrics/endpoints incl. estimated profit with partial-coverage warnings) + Tax reports (collected, by jurisdiction, manual overrides, exempt customer report) + TaxExemption CRUD. 210/210 backend tests. Phase 7d pending. |
...
**Register last updated:** 2026-07 — EC7 phase 7c delivered (permanent operational Expense system with stable-key categories + rename-preserving history + integer-cents totals + archive/void lifecycle + receipt attachments via EC2 FileRecord reuse; canonical labeled-basis Finance Dashboard with 17 metrics/endpoints — every metric carries an explicit `basis` label, refunds never silently netted, estimated profit warns on partial cost coverage, empty months returned as zero, bounded aggregation; Tax reports honoring Invoice tax snapshots — collected total, by-jurisdiction rollup (Invoice.tax_jurisdiction_snapshot → Customer.state), manual-override report, exempt-customer report; TaxExemption CRUD with effective_from/effective_to check-at-date). Backend 210/210 green (20 new phase-7c scenarios + 190 baseline; 0 regressions). Master plan Appendix A.4 (REVISED 2026-07) commercial authority remains locked and assigned to EC11/EC12/EC13; **no commercial code lands in EC7**. EC7 remains **IN PROGRESS**; phase 7d (Curated reports + Custom Report Builder foundation + exports + `testing_agent_v3_fork` full-EC7 frontend regression) remains. Stop before Phase 7d per owner directive. EC3.1 + EC6.3 still visible + pending. EC14 gates on EC11 / EC12 / EC13 completion.
