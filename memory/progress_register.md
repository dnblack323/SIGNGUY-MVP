...
| **EC6** | Asset Library, Proofs, Signatures, and Customer Portal | **COMPLETE** | Backend 154/154 tests pass (11 new EC6 + 143 EC1–EC5 regression). Delivered: Asset Library metadata (documents + versions) layered over existing FileRecord; scoped public-action + magic-link tokens (SHA-256 at rest, single-use, revocable, action+parent-bound); portal JWT separation (`sub_scope="portal"`, disjoint dependency graph, cross-token rejection unit-tested); portal customer routes for Quotes/Orders/Invoices/Documents/Proofs/Messages/Profile with tenant+customer scoping on every query; portal identity n:1 with backend-authoritative permission bundles (5 presets); staff surfaces for Proofs (create/version/transition), Approvals (dual-parent incl. work_order_summary), Signature Requests + Signatures; Public single-action endpoints for proof approve/request_changes, sign, quote_view, invoice_view, quote request, customer intake; portal messages routed through existing email service (no new messaging schema); portal shell + login/magic-link/verify + list pages mounted outside staff `<AppShell>` at `/portal/*`; public token pages at `/p/*`. Portal payment UI + signed-PDF regeneration deferred (backend endpoints ready). Evidence: `/app/evidence/EC6_evidence.md`. |
...
**Register last updated:** 2026-02 — EC6 marked COMPLETE (backend 154/154 + portal shell + public single-action pages). Awaiting owner prompt to begin EC7.
