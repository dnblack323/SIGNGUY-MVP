...
| **EC8** | Team, Scheduling, Time, Payroll, Employee Portal, Equipment Training & Certification | **PREFLIGHT COMPLETE — AWAITING OWNER APPROVAL** | Documentation-only preflight delivered: `/app/preflight/EC8_TEAM_SCHEDULING_TIME_PAYROLL_EMPLOYEE_PORTAL_PREFLIGHT.md`. Source inspection confirms the EC8 domain is a clean slate (no employee/payroll/schedule/equipment models, routers, services, or collections exist); `core/permissions.py` has unused, differently-named scaffolded permission values from EC1 that must be reconciled (not duplicated); `PortalIdentity` (EC6) requires additive extension (optional `employee_id` + `portal_type` discriminator) to support Employee Portal — no second portal-identity system. Equipment/Training/Certification added as owner-locked permanent EC8 scope (master plan Appendix A.5). Proposed 8a–8f phasing, permission set, and architecture recorded. No implementation code written; no full test suite run per preflight-only directive. Awaiting owner's ask_human gate answers before Phase 8a begins. |
...
**EC8 preflight delivered. Awaiting owner's ask_human gate answers before Phase 8a implementation begins.**

- Do NOT write EC8 implementation code until the owner answers the ask_human gate (source findings review, reuse/rebuild confirmation, permission set, phasing, repository-class question, migration risks).
- Do NOT begin EC9.
- Do NOT invoke or rerun the EC0–EC7 implementation prompts.

---

**Register last updated:** 2026-07 — **EC8 PREFLIGHT COMPLETE.** Documentation-only pass produced `/app/preflight/EC8_TEAM_SCHEDULING_TIME_PAYROLL_EMPLOYEE_PORTAL_PREFLIGHT.md` covering feature purpose, existing source map (clean slate — no employee/payroll/schedule/equipment code exists; unused EC1-scaffolded permission values and disabled nav placeholders found), current/target data models, problems found, keep/rebuild/remove, proposed architecture (single Employee/TimeEntry/PayPeriod/PayrollTransaction ledger model, `PortalIdentity` additive extension for Employee Portal, Work Order certification enforcement matrix), dependencies, risks, acceptance tests, proposed permission set, and 8a–8f phasing. Equipment/Training/Certification added as owner-locked permanent EC8 scope via master plan Appendix A.5. No implementation code written; no full test suite run (preflight-only directive honored). EC3.1 + EC6.3 remain visible + pending (must land before EC14). Awaiting owner approval before Phase 8a.
