"""EC15 - tenant-scoped Wrap Lab repository helpers."""
...
class WrapLabRepository:
    ...
    async def list(
        ...
        *,
        ...
        cursor = self.collection.find(q, {"_id": 0}).sort(sort or [("updated_at", -1)]).skip(skip).limit(limit)
    ...
    async def update(self, *, tenant_id: str, entity_id: str, updates: dict[str, Any]) -> Optional[dict]:
        ...
        result = await self.collection.update_one({"tenant_id": tenant_id, "id": entity_id}, {"$set": prepare_for_mongo(updates)})
