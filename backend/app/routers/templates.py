...
async def list_templates(
    ...
    return {"items": await template_service.list_templates(tenant_id=user["tenant_id"], template_type=template_type, active=active)}


@router.get("/{template_id}")
async def get_template(template_id: str, user: dict = Depends(require_permission(Perm.TEMPLATE_READ))) -> dict:
    try:
        return await template_service.get_template(tenant_id=user["tenant_id"], template_id=template_id)
    except TemplateError as ex:
        _raise(ex)
    except DecisionRoomError as ex:
        raise HTTPException(status_code=_ERROR_STATUS.get(ex.code, 400), detail=str(ex))
