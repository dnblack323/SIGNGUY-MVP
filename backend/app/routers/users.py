...
from ..core.user_output import EXTERNAL_USER_PROJECTION, serialize_external_user
...
async def list_users(user: dict = Depends(require_permission(Perm.USER_READ))) -> list[dict]:
    cursor = db.users.find({"tenant_id": user["tenant_id"]}, EXTERNAL_USER_PROJECTION).sort("created_at", 1)
    return [serialize_external_user(doc) async for doc in cursor]
...
async def create_user(payload: UserCreateIn, user: dict = Depends(require_permission(Perm.USER_WRITE))) -> dict:
    ...
    out = serialize_external_user(u.model_dump())
    return out
...
async def update_user(user_id: str, payload: UserUpdateIn, user: dict = Depends(require_permission(Perm.USER_WRITE))) -> dict:
    ...
    doc = await db.users.find_one({"id": user_id, "tenant_id": user["tenant_id"]}, EXTERNAL_USER_PROJECTION)
    return serialize_external_user(doc)
