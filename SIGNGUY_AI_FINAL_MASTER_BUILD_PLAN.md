# SignGuy AI — Final Master Build Plan

**Document date:** 2026-02 (Prompt 4 authoritative construction manual)
**Author:** E2 planning agent — no application code changed
...
SignGuy AI ships as **nine implementation checkpoints (CP1–CP9)** delivering the permanent commercial product on top of the existing MVP foundation. The MVP already contains a production-safe Auth / Tenants / Permissions / Customers / Quotes / Orders / Work Orders / Invoices / Object Storage / Pricing Foundation & Calculator surface. The remaining work is a mixture of targeted extensions, donor-code extractions (FEB financial logic; REB Settings / Communications / DocuLink / Wrap Lab / Platform Admin / AI catalog / Quote+Order shapes; ORIG signatures / approvals / portal / webstore feature discovery), and greenfield builds (portal identity, background jobs, feature entitlements, AI credit ledger, custom report builder, global search, tax provider boundary).
...
**First checkpoint recommended for implementation:** CP1 — Product Rules, Security Guards, and Money Policy Landing.

**Checkpoint counts and totals** (see Part 34 for exact totals):
- 9 checkpoints (CP1–CP9).
...
| 20 | Report-builder scope | (a) Curated + custom builder; (b) Curated only; (c) Full BI | **(a)** Curated + staged custom builder | ORIG has reports | Full BI = late; curated only = churn | Reports | CP5 | No | Yes (CP5 GA) | No (post-launch acceptable for advanced builder) | Pending | REQUIRES OWNER ANSWER |
...
# PART 5 — FINAL CHECKPOINT ARCHITECTURE

## 5.1 The Nine Checkpoints
...
## 5.2 Rationale for Nine Instead of Eight
...
| Settings Framework | Not built | CP2 | REB `models/settings.py` + `services/settings.py` | REF | All modules with tenant config | Namespaced settings; audit | tenant scoped | settings change events | Yes |
...
| Notifications (in-app) | Not built | CP2 | REB `notifications.py` | REF | All modules | | tenant + user scoped | notification events | Yes |
...
| Global Search | Not built | CP8 (or later) | New | REBUILD | All modules | Tenant scoping | tenant scoped | | No (post-launch acceptable) |
...
| Global Search | Cross-cutting | (bar) | I | REQ-DEP | New | New | REBUILD | N | — | Modules with search | CP8 | U,I | SearchDocs | N (post-launch OK) |
...
| PF9 | Wrap Lab portal projection | REB workflow engine + ORIG portal fragments | REB+ORIG | REB `services/wrap_workflow.py`, ORIG `routes/wrap.py` | Which stages project to portal? | DocuLink,Approvals,Signatures,Portal Auth | wrap_project,wrap_stage | /api/wrap/* | wrap_projects,wrap_stages | Wrap Lab pages + Customer Portal projection | (none) | wrap:*, portal | allowlist enforcement | REF | CP7 |
...
- **Store types:** School spirit, Team gear, Fundraiser, Corporate merch, Custom.
...
**11-stage workflow** (per REB `services/wrap_workflow.py`):

1. Inquiry & Intake.
2. Consultation & Vehicle Info.
3. Measurements & Inspection.
4. Quote (uses core Quote system + Wrap-specific metadata).
5. Design workflow (proofs + approvals).
6. Contract signature (uses core Signatures).
7. Deposit / Payment scheduling.
8. Material acquisition (may link to Inventory & Purchasing).
9. Production.
10. Installation.
11. Aftercare.
...
`<from Part 8 per module>`

**PROHIBITIONS:**
- Do not wholesale-copy donor files.
...
# PART 30 — CHECKPOINT DETAIL PAGES
...
- **First implementation checkpoint recommended:** **CP1 — Product Rules, Security Guards, and Money Policy Landing.** CP1 is blocked pending owner ratification of Decisions 1, 2, 4 (SendGrid fail-closed direction), and Decision 24 (LOCKED — already resolved). Decisions 3 and 5 are recommended but do not strictly block CP1.
- **Code changes performed in this document:** **NONE.**
- **Next action:** Owner ratification of Decisions 1, 2, 3, 4 (and confirmation of Decision 5 archive-timing recommendation), followed by the CP1 implementation prompt using the template in Part 29.

**Exact next action:** Present Decisions 1, 2, 3, 4 to the owner for approval. On approval, invoke the CP1 implementation prompt from Part 29 with `<CP_NUMBER>=CP1` and `<CP_NAME>=Product Rules, Security Guards, and Money Policy Landing`.
