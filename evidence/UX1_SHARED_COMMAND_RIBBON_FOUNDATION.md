...
## 2026-07-24 Shell Regression Restoration on CODEX-AI-BRANCH

### Regression Cause

- VERIFIED_REPOSITORY_FACT: `CODEX-AI-BRANCH` was created from `main` at `b06589e4ba71d4296a7986c7ca918af84e3607d8`.
- VERIFIED_REPOSITORY_FACT: That commit did not include the corrected UX1 shell/ribbon work from `CODEX-ux1-branch`.
- VERIFIED_REPOSITORY_FACT: The active branch rendered the legacy `FlyoutPanel` shell from `frontend/src/components/app-shell/AppShell.jsx` and the EC1 flyout navigation contract from `frontend/src/lib/navigation.js`.
- VERIFIED_REPOSITORY_FACT: The restored shell files were selectively brought forward from `CODEX-ux1-branch`, then updated to the owner-approved `Shop Operations -> Sales -> Intake / Customers / Quotes / Orders` hierarchy.

### Restored Structure

- PASS: The permanent sidebar is dark, collapsible, icon-based when collapsed, and contains only main application areas.
- PASS: Sidebar flyout mega-menus are removed; browser DOM checks reported `flyoutCount=0` across inspected routes.
- PASS: The Quick Access Toolbar is present above workspace navigation and includes global search plus New Customer, New Quote, New Order, Pricing, New Task, Calendar, and Assistant.
- PASS: Shop Operations top navigation shows Overview, Sales, Production, Materials, Webstores, Wrap Lab, and Shop Schedule, with overflow handled by the existing More menu only when required by width.
- PASS: Sales pages render page tabs for Intake, Customers, Quotes, and Orders.
- PASS: Intake, Customers, Quotes, Orders, and Work Orders render a contextual command ribbon above the compact page header.
- PASS: The large main-content `SignGuy AI` header is not rendered.

### Browser Routes Inspected

- ROUTE_VERIFIED: `/` dashboard, expanded sidebar.
- ROUTE_VERIFIED: `/` dashboard, collapsed sidebar.
- ROUTE_VERIFIED: `/intake` Sales -> Intake.
- ROUTE_VERIFIED: `/customers` Sales -> Customers.
- ROUTE_VERIFIED: `/quotes` Sales -> Quotes.
- ROUTE_VERIFIED: `/orders` Sales -> Orders.
- ROUTE_VERIFIED: `/work-orders` Production.

### Screenshots Produced

- SCREENSHOT: `evidence/screenshots/shell-restoration/dashboard-expanded.png`.
- SCREENSHOT: `evidence/screenshots/shell-restoration/dashboard-collapsed.png`.
- SCREENSHOT: `evidence/screenshots/shell-restoration/sales-intake.png`.
- SCREENSHOT: `evidence/screenshots/shell-restoration/sales-customers.png`.
- SCREENSHOT: `evidence/screenshots/shell-restoration/sales-quotes.png`.
- SCREENSHOT: `evidence/screenshots/shell-restoration/sales-orders.png`.
- SCREENSHOT: `evidence/screenshots/shell-restoration/production-work-orders.png`.

### Verification Commands

- PASS: `npm.cmd test -- --runTestsByPath src/__tests__/AppShellNavigation.test.jsx src/__tests__/OrdersCustomersRibbon.test.jsx src/__tests__/CommandRibbon.test.jsx src/__tests__/ShopSchedulePage.test.jsx --watchAll=false` from `frontend/` -> 4 suites passed, 18 tests passed.
- PASS: `git diff --check` -> no whitespace errors; Git reported CRLF conversion warnings only.

### Remaining Risks

- REMAINING_RISK: `/work-orders` rendered the corrected Production shell and ribbon but showed no work-order rows under the current filter in this local sample dataset.
- REMAINING_RISK: This restoration has not been committed or pushed; it remains in the working tree for owner review.
