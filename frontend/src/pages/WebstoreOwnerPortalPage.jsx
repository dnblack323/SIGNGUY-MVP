...
import { CheckCircle2, FileText, FileUp, MessageSquare, Save, Send } from "lucide-react";
...
export default function WebstoreOwnerPortalPage() {
  ...
  const [changeRequest, setChangeRequest] = useState({ category: "general", comment: "" });
  function load() {
    Promise.all([
  ...
  async function requestChanges() {
    ...
    try {
      await portalApi.post(`/portal/webstores/${webstoreId}/launch-packets/${data.launch_packet.id}/request-changes`, changeRequest);
  ...
  async function acceptTerms() {
    try {
      await portalApi.post(`/portal/webstores/${webstoreId}/terms/accept`, { terms_version: data.current_terms_version });
      ...
      {data.launch_packet && (
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="font-medium">Version {data.launch_packet.version || 1}</div>
                <div className="text-xs text-muted-foreground capitalize">{String(data.launch_packet.status).replace(/_/g, " ")}</div>
              </div>
              <Badge variant="outline">{data.launch_packet.delivery_status || "portal"}</Badge>
            </div>
            <div>{data.launch_packet.promotion_copy || "Launch packet is ready for approval."}</div>
            <div className="rounded border p-3 space-y-2" data-testid="portal-launch-packet-products">
              {(data.launch_packet.snapshot?.products || []).map((product) => (
                <div key={product.packet_ref || product.id} className="flex items-center justify-between gap-3">
                  <div>
                    <div className="font-medium">{product.name}</div>
                    <div className="text-xs text-muted-foreground">{product.description || product.product_type}</div>
                  </div>
                  <span className="font-medium">{centsToDollarsString(product.selling_price_cents)}</span>
                </div>
              ))}
            </div>
            <div className="rounded border p-3 text-xs" data-testid="portal-readiness-summary">
              {(data.readiness_summary || []).map((gate) => (
                <div className="flex justify-between gap-3" key={gate.key}>
                  <span>{gate.owner_wording}</span>
                  <Badge variant={gate.state === "ready" ? "secondary" : "outline"}>{gate.state}</Badge>
                </div>
              ))}
            </div>
            <div className="flex flex-wrap gap-2">
              <Button disabled={data.launch_packet.status === "owner_approved"} onClick={approve} data-testid="portal-approve-packet"><CheckCircle2 className="size-4 mr-2" />Approve packet v{data.launch_packet.version || 1}</Button>
            </div>
            <div className="rounded border p-3 space-y-2" data-testid="portal-change-request">
              <div className="font-medium">Request changes</div>
              <select className="border rounded px-2 py-1 text-sm" value={changeRequest.category} onChange={(e) => setChangeRequest({ ...changeRequest, category: e.target.value })}>
                {["branding", "product", "price", "description", "artwork", "mockup", "variant", "personalization", "fulfillment", "availability", "policy", "general"].map((category) => <option key={category} value={category}>{category.replace(/_/g, " ")}</option>)}
              </select>
              <Textarea rows={3} value={changeRequest.comment} onChange={(e) => setChangeRequest({ ...changeRequest, comment: e.target.value })} placeholder="Tell the shop what needs to change." data-testid="portal-change-request-comment" />
              <Button variant="outline" disabled={!changeRequest.comment.trim()} onClick={requestChanges} data-testid="portal-request-changes"><MessageSquare className="size-4 mr-2" />Send change request</Button>
            </div>
          </CardContent>
        </Card>
      )}
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          <div className="rounded border p-3" data-testid="portal-terms-version">
            <div className="font-medium">Version {data.current_terms_version || "webstore_terms_2026_07"}</div>
            <div className="text-xs text-muted-foreground">{data.terms_acceptance ? `Accepted ${new Date(data.terms_acceptance.accepted_at).toLocaleString()}` : "Terms acceptance is separate from packet approval."}</div>
          </div>
          <Button disabled={!!data.terms_acceptance} onClick={acceptTerms} data-testid="portal-accept-terms"><FileText className="size-4 mr-2" />Accept current Terms</Button>
        </CardContent>
      </Card>
      {(data.change_requests || []).length > 0 && (
        <Card>
          <CardHeader><CardTitle className="text-base">Change Request History</CardTitle></CardHeader>
          <CardContent className="rounded border divide-y p-0" data-testid="portal-change-request-history">
            {data.change_requests.map((request) => (
              <div key={request.id} className="p-3 text-sm">
                <div className="flex items-center justify-between gap-2"><span className="font-medium capitalize">{request.category}</span><Badge variant="outline">{request.status}</Badge></div>
                <div className="text-xs text-muted-foreground">{request.owner_comment}</div>
                {(request.owner_visible_history || []).slice(-2).map((item, index) => <div key={`${request.id}-${index}`} className="mt-1 text-xs">{item.message}</div>)}
              </div>
            ))}
          </CardContent>
        </Card>
