  ...
  return (
    <Routes>
      <Route element={<AppShell />}>
        ...
        <Route path="/quotes" element={<div>Quotes content</div>} />
        ...
        <Route path="/inventory" element={<div>Materials content</div>} />
        <Route path="/webstores" element={<div>Webstores content</div>} />
        <Route path="/wrap-lab" element={<div>Wrap Lab content</div>} />
...
test("renders only primary categories in the sidebar and Shop Operations modules in top navigation", () => {
  ...
  expect(within(topNav).getByRole("link", { name: "Overview" })).toBeInTheDocument();
  expect(within(topNav).getByRole("link", { name: "Sales" })).toHaveAttribute("aria-current", "page");
  expect(within(topNav).getByRole("link", { name: "Production" })).toBeInTheDocument();
  expect(within(topNav).getByRole("link", { name: "Materials" })).toHaveAttribute("href", "/inventory");
  expect(within(topNav).getByRole("link", { name: "Webstores" })).toBeInTheDocument();
  expect(within(topNav).getByRole("link", { name: "Wrap Lab" })).toBeInTheDocument();
...
test("separates Shop Schedule and Team Schedule navigation destinations", async () => {
  ...
  await user.click(screen.getByRole("link", { name: "Shop Schedule" }));
  expect(screen.getByTestId("category-top-nav")).toHaveAttribute("data-category-key", "shop-operations");
  expect(screen.getByRole("link", { name: "Shop Schedule" })).toHaveAttribute("aria-current", "page");
...
test("top navigation hides permission-blocked modules and keeps remaining modules reachable", () => {
  mockPermissions = ["intake:read"];
  renderWithProviders(<ShellRoutes />, { route: "/orders" });

  const topNav = screen.getByTestId("category-top-nav");
  expect(within(topNav).getByRole("link", { name: "Sales" })).toBeInTheDocument();
...
test("top navigation condenses lower-priority modules into a labeled More menu", async () => {
  ...
  expect(screen.getByTestId("module-nav-shop-schedule-overflow")).toHaveTextContent("Shop Schedule");
