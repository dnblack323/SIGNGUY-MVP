...
- Pure adapter: `backend/pricing_engine/adapters/legacy_result_adapter.py`.
- SaaS compatibility adapter: `backend/app/services/pricing_engine_adapter.py`.
- Additive API field: `pricing_engine_result`.
- DTO version: `pricing_engine_cents_first_compatibility_9il_v1`.
- Legacy source: `legacy_saas_calculator_v1` through `app.services.pricing.calculate_pricing`.
- Money conversion boundary: adapter-only `Decimal(str(legacy_value))` to integer cents with `ROUND_HALF_UP` and rounding policy `pricing_rounding_v1_round_half_up_final_cents`.
- Normalized envelope includes category/status, selling/true-cost/suggested/profit cents, selected-method amount cents, method-row amount cents, breakdown amount cents, component amount cents, version/evidence metadata, adapter-source evidence, warnings/errors, and persistent side-effect evidence.
- `/pricing/calculate` now returns the additive `pricing_engine_result` envelope for successful legacy calculations while preserving existing legacy fields and status behavior.
- All nine shared starter fixtures execute through the 9I-L compatibility adapter against the single expected-values source in the Phase 9I-K fixtures.
- The pure adapter imports without application startup and has no FastAPI, Mongo, auth, tenant, network, or app-service dependencies.
- No calculator formula service, frontend file, persistence model, migration, saved-calculation behavior, snapshot behavior, Quote/Order orchestration, Webstore behavior, Wrap Lab behavior, data migration, pure line engine, pure document engine, standalone adapter, portable configuration, or licensing work changed.
- Verification passed: Phase 9I-L focused tests `16 passed, 6 warnings`; Phase 9I-K fixture tests `29 passed`; Phase 9I-J contract tests `23 passed`; money-policy and existing Phase 9I contract regressions `32 passed`; category-output and Banner regressions `33 passed`; pricing-router/category compatibility regressions `40 passed, 6 warnings`; backend compile/import validation passed; `git diff --check` passed with CRLF warnings only.
- Full all-adapter parity is not complete. Phase 9I remains open; this 9I-L record is superseded by the 9I-M implementation record below.
...
- Model change: `backend/app/models/pricing_saved_calculation.py` adds additive `pricing_engine_result` and `saved_calculation_contract_version`.
- Service change: `backend/app/services/pricing_saved_calculations.py` now creates new saved calculations through `calculate_pricing_with_cents_first_envelope`, stores one canonical `pricing_engine_result`, copies top-level compatibility `selling_price_cents` from that normalized result, and keeps `calculation_result` as the legacy calculator payload without duplicating the envelope inside it.
- Canonical saved normalized result field: `pricing_engine_result`.
- Saved-calculation contract version: `pricing_saved_calculation_money_contract_9im_v1`.
- New-write authority: fresh successful Phase 9I-L `pricing_engine_result` with integer non-boolean `selling_price_cents`; failed, unavailable, malformed, or price-missing results cannot be saved.
- Legacy reader hierarchy: stored successful `pricing_engine_result`; then valid integer `selling_price_cents`; then read-only Decimal/ROUND_HALF_UP adaptation from stored legacy calculation result or legacy selling-price field. Legacy reads do not update MongoDB, `updated_at`, or audit records.
- Duplicate behavior: normalized records preserve normalized evidence exactly; legacy records create a new additive normalized duplicate from stored historical evidence without recalculating current settings and without mutating the source.
- Recalculate/reuse behavior: archived records remain blocked; active records recalculate fresh through the Phase 9I-L compatibility path, return a fresh `pricing_engine_result`, compare integer cents only, and expose the fresh result as transferable while leaving the stored historical saved calculation unchanged.
- Tenant, permission, and audit behavior remains intact for create/list/get/metadata update/duplicate/archive/restore/recalculate; all database lookups and mutations remain tenant-scoped.
...
- No calculator formula service, saved-calculation service/model, frontend file, historical backfill, data migration, Webstore behavior, Wrap Lab behavior, pure document engine, standalone adapter, portable configuration, or licensing work changed.
- Full all-adapter parity is incomplete. Phase 9I remains open. Phase 9I-O is implemented and verified; Phase 9I-P is next and has not started.

### Phase 9I-O - Pure line-level calculator extraction

- Status: implemented and verified on 2026-07-28.
- Scope: Move formula dispatch into `backend/pricing_engine/line_engine.py` with no SaaS imports, and relocate deterministic line formula modules under `backend/pricing_engine/categories/`.
- Implemented files: `backend/pricing_engine/line_engine.py`, `backend/pricing_engine/categories/*.py`, `backend/pricing_engine/config.py`, `backend/pricing_engine/method_outputs.py`, `backend/pricing_engine/method_registry.py`, `backend/pricing_engine/pricing_contracts.py`, `backend/pricing_engine/saved_items.py`, app-service compatibility wrappers, and `backend/tests/test_ec9_phase9io_line_engine_parity.py`.
- Dependencies: 9I-J through 9I-N.
- Unchanged: formulas, owner-approved defaults, minimums, markups, rounding results, Digital Print document-level minimum orchestration, frontend behavior, saved-calculation schemas, snapshot schemas, and historical data.
- Data migration: none.
- Legacy compatibility: `app.services.pricing.calculate_pricing()` is a rollback wrapper over `pricing_engine.line_engine.calculate_line()`, and the SaaS adapter still emits the old response envelope plus the additive `pricing_engine_result`.
...
Phase 9I-U implementation record:

- Added final all-category extraction verification tests in `backend/tests/test_ec9_phase9iu_final_extraction_verification.py`.
- Added extraction parity evidence in `evidence/EC9_PHASE9I_EXTRACTION_PARITY_REPORT.md`.
- The verification discovers all nine required fixture categories from the existing Phase 9I-K fixture pack and asserts starter coverage.
- The verification proves parity across `pure_line_engine_9io_v1`, `saas_configuration_adapter_9iq_v1`, `legacy_saas_cents_first_compatibility_adapter_9il_v1`, and `standalone_portable_configuration_adapter_9it_v1`.
- The verification confirms every adapter path returns the same normalized cents-first result for every fixture without copying fixture expected money values into the Phase 9I-U test code.
- The verification preserves fixture engine version, formula version, rounding policy evidence, normalized input evidence, selected/canonical method metadata, warnings, and method rows.
- The verification confirms deterministic execution, no fixture/config mutation, no persistent-entity creation, no tenant/auth/permission/audit/entitlement/licensing/secret fields in normalized results, and no SaaS runtime imports in the pure engine or standalone harness.
...
Phase 9I-L is implemented and verified.
